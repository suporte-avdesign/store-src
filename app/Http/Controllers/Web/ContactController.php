...
class ContactController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("{$this->view}.contact-1", compact(
            'menu','types','states','profiles','json_locale','json_countries',
    ...
    public function store(ContactRequest $request)
    {
        ...
        if (Auth::id()) {
            $input['type']    = autth()->type_id;
            $input['client']  = 1;
            $input['user_id'] = autth()->id();
        } else {
            // Verificar Email
        }
        ...
        $message = view("$this->view.render.success-1", compact('name'))->render();
        $out = array(
            'result' => 'success',
            'success' => $message
        );
