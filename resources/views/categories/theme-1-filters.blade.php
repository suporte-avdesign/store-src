    ...
    <div class="filters-inner-area row">
        <div id="BASEL_Widget_Sorting" class="filter-widget widget-count-4 col-xs-12 col-sm-6 col-md-3">
            ...
            <form class="woocommerce-ordering with-list" method="get">
                <ul>
                    <li>
                        <a href="{{route('category')}}/?orderby=Padrão" data-order="order-desc" @if($orderby == 'menu_order') class="selected-order" @endif>Padrão</a>
                    </li>
                    <li>
                        <a href="{{route('category')}}/?orderby=Mais Popular" data-order="popularity" @if($orderby == 'Popular') class="selected-order" @endif>Mais Popular</a>
                    </li>
                    <li>
                        <a href="{{route('category')}}/?orderby=Classificação" data-order="rating" @if($orderby == 'Classificação') class="selected-order" @endif>Classificação</a>
                    </li>
                    <li>
                        <a href="{{route('category')}}/?orderby=Novidade" data-order="date" @if($orderby == 'Novidade') class="selected-order" @endif>Novidade</a>
                    </li>
                    <li>
                        <a href="{{route('category')}}/?orderby=Preço Baixo" data-order="price-min" @if($orderby == 'price') class="selected-order" @endif>Preço: baixo para alto</a>
                    </li>
                    <li>
                        <a href="{{route('category')}}/?orderby=Preço Alto" data-order="price-max" @if($orderby == 'price-desc') class="selected-order" @endif>Preço: alto para baixo</a>
                    </li>
                </ul>
...
<div class="basel-active-filters">
    @isset($_pjax)
        <div class="basel-clear-filters-wrapp">
            <a class="basel-clear-filters" href="{{route('category')}}/">Cancelar Filtro</a>
        </div>
    @endif
    <div class="widget woocommerce widget_layered_nav_filters">
        <ul>
            @if($orderby)
            <li class="chosen">
                <a rel="nofollow" aria-label="Remover Filtro" href="{{route('category')}}/{{$parameter}}">{{$orderby}}</a>
            </li>
            @endif
            @if($filter_color)
                <li class="chosen">
                    <a rel="nofollow" aria-label="Remover Filtro" href="{{route('category')}}/{{$parameter}}">Cor: {{$filter_color}}</a>
                </li>
            @endif
            @if($filter_size)
                <li class="chosen">
                    <a rel="nofollow" aria-label="Remover Filtro" href="{{route('category')}}/{{$parameter}}">Tam: {{$filter_size}}</a>
                </li>
            @endif
            @if($max_price)
                <li class="chosen">
                    <a rel="nofollow" aria-label="Remover Filtro" href="{{route('category')}}/{{$parameter}}">Preço de
                        <span class="woocommerce-Price-amount amount">
